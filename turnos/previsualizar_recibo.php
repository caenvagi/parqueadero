...
if (isset($_POST['turnos']) && is_array($_POST['turnos'])) {
    ...
    foreach ($agrupado as $usuario_id => $info) {
        echo "<h3>Usuario: {$info['nombre']} (ID: $usuario_id)</h3>";
        echo "<ul>";
        foreach ($info['turnos'] as $t) {
            echo "<li>Turno ID {$t['id_turno']} - Fecha: {$t['fecha_inicio']} - Valor: $ {$t['valor']}</li>";
        }
        echo "</ul>";
    ...
    echo '<form method="POST" action="generar_recibo_pdf.php">';
