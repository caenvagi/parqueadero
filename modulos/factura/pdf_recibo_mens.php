...
$fpdf = new PDF_Code128('P', 'mm', array(80, 200));
$fpdf->SetAutoPageBreak(true); //Disable automatic page break
$fpdf->AddPage('portrait', array(80, 200));
...
function cabecera($fpdf, $pdo)
{
    ...
    $fpdf->cell(70, 5, 'Parqueadero', 0, 1, 'C');
...
$query = "SELECT 
            ...
            TT.tar_tiempo,
            CA.cat_nombre,
            US.nombre as nombre_us,
            CL.nombre as nombre_cli
        FROM recibo AS RE 
        INNER JOIN usuarios AS US ON US.id = RE.usuario 
        INNER JOIN cliente AS CL ON CL.placa = RE.placa
        INNER JOIN tar_tiempo AS TT ON CL.cli_tar_tiempo = TT.tar_id_nombre
        INNER JOIN categorias AS CA ON CA.cat_id = CL.categoria
        WHERE RE.recibo_id = :recibo_id
        ORDER BY RE.recibo_id DESC LIMIT 1";

$stmt = $pdo->prepare($query);
$stmt->bindParam(':recibo_id', $recibo_id, PDO::PARAM_INT);
$stmt->execute();
$row = $stmt->fetch();
...
if (!$row) {
	...
	$fpdf->SetFont('Arial', '', 12);
    $fpdf->Cell(30, 6, 'FPAR No: ', 0, 0, 'L');	
	$fpdf->SetFont('Arial', '', 12);
    $fpdf->Cell(50, 6, $row['recibo_man'], 0, 1, 'L');
    ...
    $fpdf->Ln(0);
    $fpdf->SetFont('Arial', '', 12);
    $fpdf->Cell(30, 6, 'Propietario: ', 0, 0, 'L');
    $fpdf->SetFont('Arial', '', 12);
    $fpdf->Cell(50, 6, $row['nombre_cli'], 0, 1, 'L');
    ...
    $fpdf->Ln(0);
    $fpdf->SetFont('Arial', '', 12);
    $fpdf->Cell(30, 6, 'Fecha Inicial: ', 0, 0, 'L');
    $fpdf->SetFont('Arial', '', 12);
    ...
    $fpdf->SetFont('Arial', '', 12);
    $fpdf->Cell(30, 6, 'Fecha Final: ', 0, 0, 'L');
    $fpdf->SetFont('Arial', '', 12);
    ...
    $fecha = new DateTime($row['fechaini']);
    $mesNumero = $fecha->format('m'); // Obtener el número del mes
    $meses1 = ucfirst($meses[$mesNumero]); // Mostrar el mes en español con la primera letra en mayúscula

    $fpdf->SetFont('Arial', '', 12);
    $fpdf->Cell(30, 6, 'Periodo: ', 0, 0, 'L');
    $fpdf->SetFont('Arial', '', 12);
    $fpdf->Cell(50, 6, $meses1 , 0, 1, 'L');
    $fpdf->SetFont('Arial', '', 12);
    ...
    $fpdf->SetFont('Arial', '', 10);
    $fpdf->Cell(50, 6,$row['tar_tiempo'], 0, 1, 'L');
    ...
    $fpdf->SetFont('Arial', '', 12);
    $fpdf->Cell(50, 6, $row['nombre_us'], 0, 1, 'L');
